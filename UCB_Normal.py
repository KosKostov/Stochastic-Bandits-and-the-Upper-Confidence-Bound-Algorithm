...
def Ucb_Generic(Bandit, n):
    """A generic implementation of the Upper Confidence Bound (UCB) algorithm on a simple stochastic bandit environment.
    
    Args:
        Bandit (object): An object representing the bandit environment. It should have methods like `K()`, `pull()`, 
                         `expected_regret()`, and member variables like `cum_exp_reg`, `expected_regrets`, 
    ...
    for t in range(n):
        ...
        if t < arms:
            ...
            # Iterate through all the arms to calculate UCB values
